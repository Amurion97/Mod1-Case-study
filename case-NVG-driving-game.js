...
class HTMLElements {
    ...
    setHidden() {
        ...
        this.htmlElement.classList.add("visually-hidden");
    }

    removeHidden() {
        // this.htmlElement.removeAttribute("hidden");/
        this.htmlElement.classList.remove("visually-hidden");
    }
}
...
class field extends HTMLElements {
    ...
    init() {
        ...
        // this.styles.margin = "0 0 0 100px";
        // this.styles.left = "100px";
    }
...
class ScoreBoard extends HTMLElements {
    constructor(htmlID, score, level) {
        ...
        this.styles.fontSize = "35px";
        this.styles.margin = "0 0 -30px 0";
        this.styles.fontWeight = "900";
        this.update();
    }

    update() {
        this.htmlElement.innerHTML = `<p>SCORE: ${this.score} - LEVEL: ${this.level}</p>`;
    }
...
if (typeof (Storage) !== "undefined") {
    ...
    leaderBoardContent += `Your highest score is ${user.highestScore}`;
    createLeaderBoard();
} else {
    // Sorry! No Web Storage support..
    document.getElementById("leader-board").innerHTML = "Your browser does not support saving scores for comparison!";
}
// document.getElementById("leader-board").innerHTML = leaderBoardContent;
...
function main() {
    ...
    let refreshIntervalId = setInterval(function () {
        if (timer > 0) {
            ...
            pinkCar.styles.left = parseInt(pinkCar.styles.left) + pinkCar.speedX * 3 + "px";
            pinkCar.styles.top = parseInt(pinkCar.styles.top) + pinkCar.speedY * 3 + "px";

            if (pinkCar.checkAllOverlap(obstacles.brownCrates) >= 0 || pinkCar.checkAllOverlap(obstacles.walls) >= 0) {
                alert("Game over!");
                updateHighestScore();
                createLeaderBoard();
                console.log(pinkCar.checkAllOverlap(obstacles.brownCrates))
            ...
            if (starCollected >= 0) {
                scoreBoard.score++;
                scoreBoard.update();
                if (scoreBoard.score === (scoreBoard.level **2) * 2) {
                    levelUp();
                    timer = 3000;
                }
...
function resetObstaclesPosition() {
    ...
    obstacles.brownCrates.forEach(function (value, index, array) {
        value.initObstacle();
    })
...
function createLeaderBoard() {
    let user = JSON.parse(localStorage.getItem(username));
    let result = `<p>Hello <strong>${user.name}</strong>!</p>`
        + `</p><p>Your highest score is <strong>${user.highestScore}</strong></p>`;
    result += "<h3 class='text-center leader-board'>LEADER BOARD</h3>";
    result += "<table class=\"table table-bordered border-success table-success table-striped\">";
    result += "<tr><th>No.</th><th>Username</th><th>Score</th></tr>";

    let allScores = getAllScoresSorted();
    for (let i = 0; i < allScores.length; i++) {
        result += `<tr><td>${i + 1}</td><td>${allScores[i].name}</td><td>${allScores[i].score}</td></tr>`
    }

    result += "</table>";

    document.getElementById("leader-board").innerHTML = result;
}

function getAllScoresSorted() {
    let arr = [];
    for (let i = 0; i < localStorage.length; i++) {
        let user = JSON.parse(localStorage.getItem(localStorage.key(i)));
        arr.push({name: user.name, score: user.highestScore});
    }
...
function updateHighestScore() {
    let user = JSON.parse(localStorage.getItem(username));
    if (scoreBoard.score > user.highestScore) {
        user.highestScore = scoreBoard.score;
    }
